"""
Train MobileNetV3-Large on C-NMC Dataset — PyTorch + GPU Edition.
...
NUM_WORKERS = 4
PHASE1_EPOCHS = 5
PHASE2_EPOCHS = 15   # cumulative
PHASE3_EPOCHS = 50   # cumulative
LR = 1e-4
...
def get_dataloaders(train_dir, val_dir):
    ...
    # Class weights for imbalanced data (sqrt-balanced)
    class_counts = np.bincount([label for _, label in train_dataset.samples])
    total = sum(class_counts)
    class_weights = torch.tensor([
        (total / (2 * c)) ** 0.5 for c in class_counts
...
# ============ MODEL ============
def build_model():
    """MobileNetV3-Large with custom 2-class head."""
...
def train():
    print("=" * 60)
    print("  MobileNetV3-Large — C-NMC PyTorch Training (GPU)")
    ...
    # Build model
    print("\n[3/5] Building MobileNetV3-Large...")
    ...
    # CSV log
    log_path = os.path.join(OUTPUT_DIR, "training_log_pytorch.csv")
    ...
    }, os.path.join(OUTPUT_DIR, "mobilenetv3_large_cnmc_best.pth"))

    # Export to ONNX
    model.eval()
    dummy_input = torch.randn(1, 3, IMG_SIZE, IMG_SIZE).to(DEVICE)
    onnx_path = os.path.join(OUTPUT_DIR, "mobilenetv3_large_cnmc.onnx")
    torch.onnx.export(
    ...
    try:
        ...
        tflite_path = os.path.join(OUTPUT_DIR, "mobilenetv3_large_cnmc.tflite")
