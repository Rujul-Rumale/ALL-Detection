...
class CNMCDataset(Dataset):
    """
    CPU dataset using PyTorch native torchvision.transforms.v2.
    This guarantees thread-safety for DataLoader workers on Windows 
    (unlike OpenCV or Albumentations).
    """
    def __init__(self, image_label_pairs, target_size: int, is_train: bool = True):
        ...
        # v2 Pipeline runs entirely on CPU dynamically inside __getitem__
        if self.is_train:
            self.transform = v2.Compose([
                ...
                # Blur
                v2.RandomApply([v2.GaussianBlur(kernel_size=(3, 5), sigma=(0.1, 2.0))], p=0.2),

                # Dropout (Erasing) - Cutout
            ...
            self.transform = v2.Compose([
                v2.ToImage(),
                v2.ToDtype(torch.float32, scale=True),
                v2.Resize(target_size + 32, interpolation=v2.InterpolationMode.BILINEAR, antialias=True),
    ...
    def __getitem__(self, idx):
        ...
        # Use PIL exclusively to avoid OpenCV multiprocessing locks
        image = Image.open(fpath).convert("RGB")
        
        # Returns float32 [0.0, 1.0] CHW tensor
        tensor = self.transform(image)
        
        return tensor, label

        # (Placeholder for removed numpy/cv2 code)
...
class CUDAPrefetcher:
    ...
    def __init__(self, loader, device: torch.device):
        ...
        self.next_input: torch.Tensor | None = None
    ...
    def preload(self):
        try:
            self.next_input, self.next_labels = next(self._iter)
        ...
        with torch.cuda.stream(self.stream):
            # Already float32, just copy to device
...
def train_one_epoch(model, loader, criterion, optimizer, scaler,
    ...
    for images, labels in loader:
        # Move CPU tensor to GPU asynchronously
...
def main():
    ...
    # Prefetcher disabled due to synchronization deadlocks with heavy CPU augs
